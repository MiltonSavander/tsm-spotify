	...
	useEffect(() => {
		const getData = async () => {
			...
			try {
				...
				setPlaylistInfo({ image: playlistDetails.body.images[0].url, name: playlistDetails.body.name });
	...
	return (
		<Box id="Playlist__page" sx={{ backgroundColor: 'background.paper', flex: 1, overflowY: 'autos' }}>
			...
			>
				...
				<Box>
					<Typography sx={{ fontSize: 12, fontWeight: 'bold', color: 'text.primary' }}>Playlists</Typography>
					{status.isLoading ? (
						<Skeleton variant="text" sx={{ fontSize: { xs: 24, md: 72 }, width: 300 }} />
					) : (
						<Typography sx={{ fontSize: { xs: 24, md: 72 }, fontWeight: 'bold', color: 'text.primary' }}>
							{playlistInfo?.name}
						</Typography>
					)}
				</Box>
